...
function BookMockup() {
  return (
    <div className="relative w-[180px] h-[240px] mx-auto" style={{ perspective: '900px' }}>
      <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 w-[70%] h-5 rounded-full blur-xl"
      ...
      <div className="w-full h-full" style={{ transform: 'rotateY(-15deg) rotateX(3deg)', transformStyle: 'preserve-3d' }}>
        <div className="absolute inset-0 rounded-r-lg rounded-l-sm overflow-hidden"
          style={{ background: 'linear-gradient(145deg,#0F2E4A,#0A1628)', boxShadow: '8px 12px 40px rgba(34,197,94,0.2),-2px 0 8px rgba(0,0,0,0.9)' }}>
          <div className="absolute inset-0 p-4 flex flex-col justify-between">
            <div className="h-0.5 w-full rounded-full" style={{ background: 'linear-gradient(90deg,#22C55E,#16A34A)' }} />
            ...
            <div className="flex-1 flex flex-col justify-center py-2">
              <div className="font-bold uppercase tracking-widest mb-1" style={{ fontSize: '8px', color: '#22C55E' }}>Curso de</div>
            ...
            <div className="flex justify-end">
              <div className="w-7 h-7 rounded-full flex items-center justify-center font-black text-black text-sm" style={{ background: 'linear-gradient(135deg,#22C55E,#16A34A)' }}>$</div>
            </div>
          </div>
        ...
        <div className="absolute left-0 top-0 h-full w-3 rounded-l-sm"
          style={{ background: 'linear-gradient(180deg,#22C55E,#0F7B3A)', transform: 'translateZ(-3px) translateX(-3px)', boxShadow: '-2px 0 6px rgba(0,0,0,0.8)' }} />
      </div>
    </div>
  )
}
...
function LeadForm() {
    ...
    return (
      <div className="text-center py-6">
        ...
        <a
          ...
          style={{ display: 'block', wordBreak: 'break-all', color: '#22C55E', fontSize: '12px', marginBottom: '20px', textDecoration: 'underline' }}
        >
          {ZOOM_LINK}
        </a>
        <a
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-3">
      <div>
        <input type="text" className={`input-field ${errors.name ? 'border-red-500/60' : ''}`}
          placeholder="Tu nombre completo" value={form.name} onChange={handleChange('name')} />
        {errors.name && <p className="text-red-400 text-xs mt-1">{errors.name}</p>}
      </div>
      <div>
        <input type="email" className={`input-field ${errors.email ? 'border-red-500/60' : ''}`}
          placeholder="Tu correo electrónico" value={form.email} onChange={handleChange('email')} />
        {errors.email && <p className="text-red-400 text-xs mt-1">{errors.email}</p>}
      </div>
      <div>
        <input type="tel" className={`input-field ${errors.phone ? 'border-red-500/60' : ''}`}
  ...
  return (
    <>
      ...
      <main className="min-h-screen flex items-center pt-28 lg:pt-16 relative overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 80% 60% at 50% 0%, rgba(34,197,94,0.06) 0%, transparent 60%)' }} />
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <h1 className="font-black leading-tight tracking-tight mb-3" style={{ fontSize: 'clamp(2rem,5vw,3.2rem)' }}>
                ...
                <span style={{ color: '#22C55E' }}>nunca te enseñaron:</span>
              </h1>
              <h2 className="font-black leading-none mb-5" style={{ fontSize: 'clamp(2.8rem,7vw,5rem)', color: '#22C55E', letterSpacing: '-0.02em' }}>
                EDUCACIÓN<br />FINANCIERA
              </h2>
              ...
              <div className="flex items-center gap-4 mb-8">
                <div className="px-3 py-1 rounded-lg" style={{ background: '#0F1E35' }}>
                  <span className="font-bold line-through" style={{ color: '#EF4444', fontSize: '20px' }}>$99.99</span>
                </div>
                <div>
                  <div className="flex items-baseline gap-2">
                    <span style={{ color: '#64748B', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>AHORA SOLO</span>
                    <span className="font-black" style={{ color: '#4ADE80', fontSize: '44px', lineHeight: 1 }}>$1.99</span>
                  </div>
                  <p style={{ color: '#4A5568', fontSize: '12px' }}>Pagas solo al finalizar el curso</p>
                </div>
              </div>
            ...
            {/* ── Derecha: formulario ── */}
            <div>
              <div id="form-registro" className="card overflow-hidden">
                  ...
                  <div className="flex justify-center gap-6 mt-6 flex-wrap">
                    {[
                      ...
                      <div key={i} className="flex flex-col items-center gap-1 text-center" style={{ maxWidth: '80px' }}>
                        ...
                        <span className="text-xs" style={{ color: '#2D3748' }}>{text}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex -space-x-2">
                  {['A','M','J','R','L'].map((l, i) => (
                    <div key={i} className="w-7 h-7 rounded-full border-2 flex items-center justify-center text-xs font-bold"
                      style={{
                        borderColor: '#0D1F3C',
                        background:  ['#22C55E','#16A34A','#234A8A','#244A7F','#4ADE80'][i],
                        color:       i < 2 ? '#000' : '#fff',
                      }}>
                      {l}
                    </div>
      ...
      <a href={WA_LINK} target="_blank" rel="noopener noreferrer"
        className="fixed bottom-6 right-5 z-50 flex items-center gap-2 text-white text-sm font-bold px-4 py-3 rounded-full hover:-translate-y-0.5 transition-transform"
      ...
      <footer style={{ borderTop: '1px solid #1E3A5F', background: '#07111F' }}>
        <div className="max-w-6xl mx-auto px-4 py-6 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded flex items-center justify-center font-black text-black text-xs"
              style={{ background: 'linear-gradient(135deg,#22C55E,#16A34A)' }}>B</div>
